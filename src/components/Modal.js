...
newdrop=(e)=>{
  ...
  // e.target.reset();
  // e.preventDefault()
  // this.setState({adding:[...this.state.adding,<Button data={this.state.list}/>]})
}
hanonChange=(e,index)=>{
  ...
  //   var value = this.state.list2[key];
    ...
    //     console.log("ok");
    //     this.setState({list2:this.state.list2.splice(key,1)})
    // }
// }
    // this.setState({list:this.state.list.splice(idx,1)})
    this.value=""
    // location.reload();
      // e.preventDefault();
  }
...
handlesave=(e)=>{
    ...
    //  this.props.nameofSegment(this.state.nameofSegment)
    // e.target.reset()
}
  ...
  <div class="modal-dialog modal-dialog-scrollable ">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">&gt;Segments</h5>
      ...
      <div class="modal-body">
        <p></p>
        <p >Enter the name of the segment<br/><input placeholder="Name of the Segment" onChange={this.onchange}></input></p>
        <p >
          To save your segment you need to add the schemas to build the query
        </p>
        <p >
          <div>
        {this.state.clicked && this.state.list3.map((item, i) => ( 
        <div class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups">
           <div class="btn-group mr-2" role="group" aria-label="First group">
          <Button text={item} placeholder={this.state.list3[i].Value} data={this.state.list3} style={{color:"black"}} >  </Button>
           </div>
           <div class="btn-group mr-2" role="group" aria-label="Second group">
          <button type="button" class="btn btn-danger" onClick={()=>this.delete(i)}>-</button>
          </div>
        </div>
           ))}
        ...
        <Button data={this.state.list} placeholder={this.state.placeholder} name="maindropdown" value="" hanonChange={(e)=>this.hanonChange(e)}></Button>
        
        <a onClick={this.newdrop}>+ Add New Schema</a></p>
        
      <div class="modal-footer" id="footer1">
        <button type="button" class="btn btn-danger" data-dismiss="modal">Close</button>
        <button type="button" class="btn btn-primary" onClick={this.handlesave} >Save changes</button>
      </div>
    </div>
  </div>
</div>
